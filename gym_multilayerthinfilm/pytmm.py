...
def list_snell_new(n_list, th):
    ...
    assert angles.shape == (th.shape[0], n_list.shape[0], n_list.shape[1]), (
    th.shape[0], n_list.shape[0], n_list.shape[1])
...
def T_from_t_new(pol, t, n_i, n_f, th_i, th_f):
    ...
    if pol == 's':
        ni_thi = np.real(cos(th_i) * n_i)
        nf_thf = np.real(cos(th_f) * n_f)
        return (abs(t ** 2) * ((nf_thf) / (ni_thi)))

    elif pol == 'p':
        ni_thi = np.real(conj(cos(th_i)) * n_i)
        nf_thf = np.real(conj(cos(th_f)) * n_f)
        return (abs(t ** 2) * ((nf_thf) / (ni_thi)))
...
def coh_tmm_fast(pol, n_list, d_list, th_0, lam_vac):
    ...
    * pol, n_list, d_list, th_0, lam_vac--same as input
    ...
    # of each layer
    A = exp(
        1j * delta[:, :, 1:-1])  # [lambda, theta, n], n without the first and last layer as they function as injection
    # and measurement layer
    F = r_list[:, 1:]
    # print('F: ', F.shape)
